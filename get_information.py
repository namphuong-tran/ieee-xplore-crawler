...
def get_article(airtcle_number):
    headers = {
...
def get_year(page_text):
    ex = '"publicationYear":"(.+?)"'
    article_year = re.findall(ex, page_text, re.S)
    for year in article_year:
        if year != 'true' and year != 'false':
            return int(year)
...
def get_citation_number(page_text):
    ...
    article_citation_count = re.findall(ex, page_text, re.S)
    for citation_count in article_citation_count:
        if citation_count != 'true' and citation_count != 'false':
